...
namespace WPM {
	...
	public class SenseConnectorDropdown : MonoBehaviour {
		...
		public Dropdown	SelectedOrgDropdown;
		public string[]	SelectedOrgs;
...
//			form.AddField("fieldValue", "Deloitte"); // Armanino // Empathica
		...
		public void fillDropdown(string textString) {
			textString = textString.Replace ("\"", string.Empty);
			SelectedOrgs = textString.Split ('\n');
			SelectedOrgDropdown.options.Clear ();
			foreach (string o in SelectedOrgs) {
				SelectedOrgDropdown.options.Add (new Dropdown.OptionData () { text = o });
			}
			SelectedOrgDropdown.value = 1;
			SelectedOrgDropdown.value = 0;
			SelectedOrgText.text = SelectedOrgs [0];
		}

		private void SelectedOrgDropdownValueChangedHandler(Dropdown target){
			Debug.Log ("selected ID: " + target.value);
			Debug.Log ("selected name: " + target.options[target.value].text);
			//selectOrg (target.options [target.value].text);
		}
